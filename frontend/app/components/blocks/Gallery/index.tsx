...
import type { GalleryBlockProps } from '@/app/components/blocks/types'
...
export default function GalleryBlock({ heading, subtitle, viewAllText, viewAllHref, photos = [] }: GalleryBlockProps) {
  return (
    <section className="overflow-hidden py-20 bg-white">
      ...
      <div className="relative mx-auto mt-16 hidden max-w-360 lg:block" style={{ height: '877px' }}>
      ...
      <div className="mt-12 grid grid-cols-2 gap-4 px-4 sm:px-6 lg:hidden">
        {[...Array(4)].map((_, i) => (
          <div
            ...
            style={{ height: i % 2 === 0 ? 256 : 200, background: GRADIENTS[i] }}
          >
            {photos[i] && (
              <Image
                src={photos[i]}
      ...
      <div className="mt-16 text-center">
        <a href={viewAllHref} className="inline-flex rounded-full px-8 py-3 text-sm  transition-colors bg-accent hover:bg-sand">
          {viewAllText}
        </a>
      </div>
    </section>
